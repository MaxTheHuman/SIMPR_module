...
LRESULT CALLBACK WindowProc(HWND, UINT, W PARAM, LPARAM);
int Condition(WPARAM wParam, LPARAM lParam);
int Action(WPARAM wParam, LPARAM lParam);

char szWinName[] = "Перекресток с трамваем";
...
Car *car, *p;
Car *bLBegin = NULL, *bLLast = NULL, *bLWork;
Car *bSRBegin = NULL, *bSRLast = NULL, *bSRWork;
Car *rLBegin = NULL, *rLLast = NULL, *rLWork;
Car *rSRBegin = NULL, *rSRLast = NULL, *rSRWork;
Car *tLBegin = NULL, *tLLast = NULL, *tLWork;
Car *tSRBegin = NULL, *tSRLast = NULL, *tSRWork;
Car *lLBegin = NULL, *lLLast = NULL, *lLWork;
Car *lSRBegin = NULL, *lSRLast = NULL, *lSRWork;
Trailway *trail = NULL;
...
int WINAPI WinMain(HINSTANCE hThisInst, HINSTANCE hPrevInst, LPSTR lpszArgs,
  ...
  if (!RegisterClass(&wcl))
    return 0;
  hwnd = CreateWindow(szWinName, "Перекресток с трамваем",
                      WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,
                      CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                      CW_USEDEFAULT, HWND_DESKTOP, NULL, hThisInst, NULL);
  hInst = hThisInst;
  hAccel = LoadAccelerators(hThisInst, NULL);
  MoveWindow(hwnd, (maxX - sizeX) / 2, (maxY - sizeY) / 2 - 40, sizeX + 6,
             sizeY + 45, TRUE);
...
void Car::Move() {
  ...
  if (xIncr > 0 && rect.left > 400 || xIncr < 0 && rect.right < 0 ||
      yIncr > 0 && rect.top > 400 || yIncr < 0 && rect.bottom < 0)
    work = 0;
}
  ...
  if (from == BOTTOM)
    if (to == RIGHT || to == STRAINGHT && r) {
      if (bSRBegin == bSRLast) {
        ...
        bSRBegin = bSRBegin->next;
        bSRBegin->prev = NULL;
      } else if (this == bSRLast) {
        bSRLast = bSRLast->prev;
        bSRLast->next = NULL;
      } else {
        prev->next = next;
        next->prev = prev;
    ...
    if (to == RIGHT || to == STRAINGHT && r) {
      if (rSRBegin == rSRLast) {
        ...
        rSRBegin = rSRBegin->next;
        rSRBegin->prev = NULL;
      } else if (this == rSRLast) {
        rSRLast = rSRLast->prev;
        rSRLast->next = NULL;
      } else {
        prev->next = next;
        next->prev = prev;
    ...
    if (to == RIGHT || to == STRAINGHT && r) {
      if (tSRBegin == tSRLast) {
        ...
        tSRBegin = tSRBegin->next;
        tSRBegin->prev = NULL;
      } else if (this == tSRLast) {
        tSRLast = tSRLast->prev;
        tSRLast->next = NULL;
      } else {
        prev->next = next;
        next->prev = prev;
    ...
    if (to == RIGHT || to == STRAINGHT && r) {
      if (lSRBegin == lSRLast) {
        ...
        lSRBegin = lSRBegin->next;
        lSRBegin->prev = NULL;
      } else if (this == lSRLast) {
        lSRLast = lSRLast->prev;
        lSRLast->next = NULL;
      } else {
        prev->next = next;
        next->prev = prev;
...
void Trailway::Move() {
  ...
  BitBlt(memDC, rect.left, rect.top, rect.right - rect.left,
         rect.bottom - rect.top, mDC[from], 0, 0, SRCCOPY);
  InvalidateRect(hwnd, &rect, 1);
  if (from == BOTTOM && rect.top < 240 || from == RIGHT && rect.left < 240 ||
      from == TOP && rect.bottom > 240 || from == LEFT && rect.right > 240)
    place = AFTER;
  if (xIncr > 0 && rect.left > 400 || xIncr < 0 && rect.right < 0 ||
      yIncr > 0 && rect.top > 400 || yIncr < 0 && rect.bottom < 0)
    work = 0;
...
int Condition(WPARAM wParam, LPARAM lParam) {
  ...
  case 4: // Таблица 4
    ...
    case 2: // Машина едет налево?
      if (bLWork && bLWork->to == LEFT)
        return YES;
      else
        return NO;
    case 3: // Едет ли трамвай?
      if (bLWork && bLWork->to == LEFT)
        return YES;
      else
        return NO;
  ...
  case 5: // Таблица 5
    ...
    case 4: // Позиция регулировщика № 1?
      if (pos == 1)
        ...
        return NO;
    case 5: // Позиция регулировщика № 2?
      if (pos == 2)
        return YES;
      else
        return NO;
    case 6: // Позиция регулировщика № 3?
      if (pos == 3)
        return YES;
      else
        return NO;
  ...
  case 7: // Таблица 7
    ...
    case 4: // Позиция регулировщика № 5?
      if (pos == 5)
        ...
        return NO;
    case 5: // Позиция регулировщика № 6?
      if (pos == 6)
        return YES;
      else
        return NO;
    case 6: // Позиция регулировщика № 1?
      if (pos == 1)
        return YES;
      else
        return NO;
  ...
  case 9: // Таблица 9
    ...
    case 5: // Позиция регулировщика № 2?
      if (pos == 2)
        ...
        return NO;
    case 6: // Позиция регулировщика № 5?
      if (pos == 5)
        return YES;
      else
        return NO;
  ...
  case 11: // Таблица 11
    ...
    case 5: // Позиция регулировщика № 6?
      if (pos == 6)
        ...
        return NO;
    case 6: // Позиция регулировщика № 4?
      if (pos == 4)
        return YES;
      else
        return NO;
